...
class DeepSVDD(object):
    ...
    def pretrain(self, dataset: BaseADDataset, optimizer_name: str = 'adam', lr: float = 0.001, n_epochs: int = 100,
        ...
        logger.info(f'Yahan se arha{self.finalLR}')
    ...
    def save_model(self, export_model, save_ae=True, save_de=False):
        """Save Deep SVDD model to export_model."""

        net_dict = self.net.state_dict()
        ae_net_dict = self.ae_net.state_dict() if save_ae else None
        de_net_dict = self.de_net.state_dict() if save_de else None

        torch.save({'R': self.R,
                    'c': self.c,
                    'net_dict': net_dict,
                    'ae_net_dict': ae_net_dict,
                    'de_net_dict':de_net_dict}, export_model)

    def load_model(self, model_path, load_ae=False, load_de=False):
    ...
    def save_graphs(self,graph_path):
        self.compare_AEDE(graph_path + os.path.sep + "train_comp.png")
        self.compare_train(graph_path + os.path.sep + "trainloss_comp.png")
        self.compare_test(graph_path + os.path.sep + "testloss_comp.png")

    
    def compare_AEDE(self, image_path):
        plt.figure(figsize=(20,10))        
        plt.plot(range(1,self.ae_n_epochs+1),self.loss_ae,color='red',label='Orig Autoenc')
        plt.plot(range(1,self.de_n_epochs+1),self.loss_de,color='blue',label='After SVDD Loss')
    ...
    def compare_train(self, image_path):
    ...
    def compare_test(self, image_path):
        plt.figure(figsize=(10,10))
